# Hotel Queries (ID: 1143)

**Problem Link:** [https://cses.fi/problemset/task/1143/](https://cses.fi/problemset/task/1143/)
...
int a[N];
pair <int,int> t[4*N];
pair <int,int> merge(pair <int,int> ch1,pair <int,int> ch2){
    if(ch1.F==ch2.F){
        return {ch1.F,min(ch1.S,ch2.S)};
    }
    return max(ch1,ch2);
}
void update(int pos,int el,int tl,int tr,int v){
    if(tl==tr){
        t[v] = {el,pos};
        return;
    }
    int tm = (tl+tr) >> 1;
    if(pos <= tm){
        update(pos,el,tl,tm,v+v);
    }
    else{
        update(pos,el,tm+1,tr,v+v+1);
    }
    t[v] = merge(t[v+v],t[v+v+1]);
}
pair <int,int> get(int u,int tl,int tr,int v){
    if(tl==tr){
        return t[v];
    }
    int tm = (tl+tr) >> 1;
    if(t[v+v].F >= u){
        return get(u,tl,tm,v+v);
    }
    else{
        return get(u,tm+1,tr,v+v+1);
    }
}
void solve(){
    int n,m;
    ...
    while(m--){
        int u;
        cin >> u;
        pair <int,int> ip = get(u,1,n,1);
        if(ip.F < u){
            cout << 0 << " ";
            continue;
        }
        update(ip.S,ip.F-u,1,n,1);
        cout << ip.S << " ";
    }
