# List Removals (ID: 1749)

**Problem Link:** [https://cses.fi/problemset/task/1749/](https://cses.fi/problemset/task/1749/)
...
void update(int pos,int val,int tl,int tr,int v){
    ...
    int tm = (tl+tr)/2;
    if(pos <= tm){
        update(pos,val,tl,tm,v+v);
    }
    else{
        update(pos,val,tm+1,tr,v+v+1);
    }
    t[v].f = t[v+v].f+t[v+v+1].f;
}
edge get(int k,int tl,int tr,int v){
    if(tl==tr){
        return t[v];
    }
    int tm = (tl+tr) >> 1;
    if(t[v+v].f < k){
        return get(k-t[v+v].f,tm+1,tr,v+v+1);
    }
    else{
        return get(k,tl,tm,v+v);
    }
}
...
void solve(){
    ...
    for(int i=1;i <= n;i++){
        ...
        update(j.t,0,1,n,1);
        cout << j.s << " ";
        // cout << "odnerka = " << j.f << " ans = " << j.s << " po faktu pos = " << j.t << " get[j.t] = " << get(j.t,1,n,1).s << "\n";
    }
}
