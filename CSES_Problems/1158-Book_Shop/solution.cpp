// CSES Problem 1158: Book Shop
// Link: https://cses.fi/problemset/task/1158/
// Description: You are in a book shop which sells n different books. You know the price and num...
...
void solve(){
    ...
    for(ll i=1;i <= n;i++){
        for(ll s=x;s-w[i] >= 0;s--){
            if(dp[s-w[i]] > -1){
                dp[s] = max(dp[s-w[i]]+g[i],dp[s]);
            }
        }
    }
    ll mx = 0;
    for(ll i=1;i <= x;i++)mx = max(dp[i],mx);
    cout << mx;
}
