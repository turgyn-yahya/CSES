# Prefix Sum Queries (ID: 2166)

**Problem Link:** [https://cses.fi/problemset/task/2166/](https://cses.fi/problemset/task/2166/)
...
edge merge(edge x,edge y){
    edge res;
    res.pr = max({0ll,x.sum+y.pr,x.pr});
    res.sum = x.sum+y.sum;
    return res;
}
...
edge get(ll l,ll r,ll tl,ll tr,ll v){
    if(tl > r || l > tr){
        return {0,0};
    }
    if(l <= tl && tr <= r)return t[v];
    ll tm = (tl+tr) >> 1;
    return merge(get(l,r,tl,tm,v+v),get(l,r,tm+1,tr,v+v+1));
}
void solve(){
    ll n,q;
    cin >> n >> q;
